...
//                cout << endl;
                ...
                angle = atan2( (edgepoints[i].y - edgepoints[j].y),0.000001+(edgepoints[i].x - edgepoints[j].x) ) * 180 / 3.14;
                ...
                for(k=0; k<edgepoints.size(); ++k)
                    if(dist.at<int>(i,k) <= 1.5*majoraxis || dist.at<int>(j,k) <= 1.5*majoraxis)
                        for(kk=0; kk<edgepoints.size(); ++kk)
